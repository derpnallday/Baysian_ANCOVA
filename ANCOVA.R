...
genMCMC = function( datFrm , yName="y" , xNomName="xNom" , xMetName="xMet" ,
  ...
  model {
    for ( i in 1:Ntotal ) {
      ...
      mu[i] <-a[xNom[i]] + aMet[xNom[i]]*xMet[i] #slope depends on group
    }
    ySigma ~ dunif( residSD/100 , ySD*10 ) 
    nu ~  dexp(1/30.0) 
    for ( j in 1:NxNomLvl ) { a[j] ~ dnorm( 0.0 , 1/aSigma^2 ) 
                              aMet[j] ~ dnorm( 0 , 1/(2*ySD/xMetSD)^2 ) } #slope has prior for each group
    aSigma ~ dgamma( agammaShRa[1] , agammaShRa[2] ) #prior for std
